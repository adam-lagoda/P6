...
P_pixel_Width = 94      #Pixel of object with distance D_initial #43 for fisheye
D_known_distance = 40   #Measured distance from camera to object in cm #52 for fishye
...
#----------------------------------
def example_move_to_home_position1(base):
    # Make sure the arm is in Single LEvel Servoing Mode
    base_servo_mode = Base_pb2.ServoingModeInformation()
    base_servo_mode.servoing_mode = Base_pb2.SINGLE_LEVEL_SERVOING
    base.SetServoingMode(base_servo_mode)
    ...
    action_handle = None
    for action in action_list.action_list: #search through the library of saved movement
        if action.name == "Home position":#bottlehome
            action_handle = action.handle
...
with utilities.DeviceConnection.createTcpConnection(args) as router:
    ...
    example_move_to_home_position1(base)
    time.sleep(5)
